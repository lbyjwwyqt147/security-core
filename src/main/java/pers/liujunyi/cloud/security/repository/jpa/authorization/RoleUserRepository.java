...
/***
 * 文件名称: RoleUserRepository.java
 * 文件描述: 用户角色 Repository
 * 公 司:
 * 内容摘要:
 * 其他说明:
 * 完成日期:2019年11月07日
...
public interface RoleUserRepository extends BaseRepository<RoleUser, Long> {
    ...
    @Transactional(rollbackFor = {RuntimeException.class, Exception.class})
    @Modifying(clearAutomatically = true)
    @Query(value = "update role_resource u set u.status = ?1, u.update_time = ?2, where u.user_id = ?3", nativeQuery = true)
    int setStatusByUserIds(Byte status, Date updateTime, List<Long> userIds);
